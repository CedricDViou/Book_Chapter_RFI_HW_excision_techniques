...
Within 5 to 88 MHz, the lowest part of the frequency band accessible from ground-based telescopes, Sky is crowded with audio and timing broadcast users allocated with a frequency channels much closer than the frequency resolution used to meet the Science requirements of Solar and Jovian observations.
This band is also the place where wide band RFI take place, generated by electric fences, spark ignition systems from internal combustion engines, power lines.

Time and frequency analysis of this band is required to be performed at a regime where temporal and spectral resolutions far exceed the observation scientific needs.  This leads to data streams and data sets much greater than required for Science.

Since 2013, a receiver \cite{lecacheux2013, 2017pre8.conf..455L} based on 80-MSps ADCs and Stratix IV FPGA implements a real-time streaming Blackman-Harris-windowed 64k-FFT followed by a classical (averaged-based) time-frequency analysis using a Welch averaged periodogram and simultaneously computes the same periodogram with a median filter (kernel size=64) applied on the frequency axis first and then on the time axis (kernel size=128) to mitigate narrow-band and wide-band RFI from observations.  The frequency resolution for RFI mitigation is as low as 1.2 kHz but is then raised to 78 kHz for Science purposes.  The time resolution for RFI mitigation is 0.8 ms but raised to 104 ms to provide a sustainable datarate for 24/7 monitoring of Sun and Jupiter.
